    ...
    <div class="panel panel-default panel-search">
      <el-form :inline="true">
        <el-form-item label="工号">
          <el-input v-model="page.query.param.employeeNo"></el-input>
        </el-form-item>
        <el-form-item label="姓名">
          <el-input v-model="page.query.param.sysAccount.realName"></el-input>
        </el-form-item>
        <el-form-item label="所属部门">
          <org-tree v-model="page.query.param.orgId" :corp-id="corpId" style="width: 100%;"></org-tree>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="search" v-keycode="'altQ'">查询(Q)</el-button>
          <el-button @click="create" v-keycode="'altN'">新增(N)</el-button>
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-row>
        <el-col :span="16">
          <v-table ref="table" :page="page" :actions="tableActions" :auto-select="true" @dataloaded="onDataloaded" @row-dblclick="edit" :table-minheight="400" action-width="100px">
            ...
            <el-table-column prop="employeeNo" label="工号" header-align="center" width="100">
            </el-table-column>
            <el-table-column prop="sysAccount.realName" label="姓名" header-align="center" width="120">
  ...
  methods: {
    ...
    del(row) {
      this.$confirm("确定删除此条记录吗?", "提示", {type: "warning"}).then(() => {
        fetch.sys.User.delete(row.id).then(rsp => {
          this.search();
          this.$message({
            type: "success",
            message: "删除成功!"
