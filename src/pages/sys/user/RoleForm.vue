  ...
  export default {
    ...
    methods: {
      submit (callback) {
        ...
        //   if (valid) {
            // if (this.entity.id) {
        fetch.sys.User.assignRoles(this.entity.id, this.roleType, this.values).then(callback)
            // } else {
      ...
      init(roleType, options){
        ...
        if (options.id) {
          ...
          fetch.sys.Role.microList({param: {corpId: this.corpId, type: roleType}}).then(rsp => {
            this.roles = rsp.data
            roleLoading = false
            this.showLoading = roleLoading || valueLoading
          })
          fetch.sys.User.findRoleIds(options.id, roleType).then(rsp => {
            this.values = rsp
            valueLoading = false
            this.showLoading = roleLoading || valueLoading
          })
