...
<script>
  import  RepairItemTypeSelect from '../../../components/widgets/RepairItemTypeSelect.vue'
  ...
  export default {
    components:{RepairItemTypeSelect, CorpSelect, OrgTree},
    data() {
      return {
    ...
    methods: {
      ...
      submitForm() {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            fetch.sys.Org.save(this.entity).then((rsp)=>{
      ...
      init(options){
        this.resetForm()
        if(options.id){
          fetch.sys.Org.get(options.id).then(rsp=>{
            this.entity = rsp
          })
        } else {
