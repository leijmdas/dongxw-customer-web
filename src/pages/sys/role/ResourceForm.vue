  ...
  export default {
    ...
    methods: {
      ...
      submit (callback) {
        ...
        //   if (valid) {
            // if (this.entity.id) {
        fetch.sys.Role.assignRoleResources(this.entity.id, this.values).then(callback)
            // } else {
      ...
      init(options){
        ...
        if (options.id) {
          ...
          fetch.sys.Resource.queryGroupAndResources().then(rsp => {
            // 所有菜单分组
          ...
          fetch.sys.Role.findResourceIds(options.id).then(rsp => {
            this.values = rsp
            valueLoading = false
            this.showLoading = srgLoading || valueLoading
          })
